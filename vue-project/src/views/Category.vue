...
const filteredNovels = computed(() => {
  ...
  console.log('过滤小说 - 小说分类示例:', novels.value.slice(0, 5).map(n => n.category))
  ...
  const filtered = novels.value.filter(novel => {
    ...
    console.log(`小说 ${novel.title} 分类: "${novel.category}" vs "${currentCategory.value}" = ${match}`)
    return match
  })
...
const fetchNovels = async () => {
  ...
  try {
    ...
    if (Array.isArray(response)) {
      ...
      console.log('小说分类示例:', response.slice(0, 3).map(n => n.category))
      novels.value = response.map(novel => ({
        id: novel.id,
        title: novel.title,
        author: novel.author,
        description: novel.description,
        cover: novel.coverImage ? `/api/novels/${novel.id}/cover` : 'https://via.placeholder.com/150x200',
      ...
      console.log('小说分类示例:', response.data.slice(0, 3).map(n => n.category))
    ...
    novels.value = [
      { id: 1, title: '斗破苍穹', author: '天蚕土豆', description: '这里是天才云集的世界...', cover: 'https://via.placeholder.com/150x200', category: '玄幻', status: '完结' },
      { id: 2, title: '庆余年', author: '猫腻', description: '一个现代青年穿越到古代...', cover: 'https://via.placeholder.com/150x200', category: '历史', status: '完结' },
      { id: 3, title: '赘婿', author: '愤怒的香蕉', description: '一个现代金融界巨头...', cover: 'https://via.placeholder.com/150x200', category: '历史', status: '连载中' },
      { id: 4, title: '凡人修仙传', author: '忘语', description: '一个普通山村小子...', cover: 'https://via.placeholder.com/150x200', category: '仙侠', status: '完结' },
      { id: 5, title: '雪中悍刀行', author: '烽火戏诸侯', description: '江湖与庙堂的纷争...', cover: 'https://via.placeholder.com/150x200', category: '武侠', status: '完结' },
      { id: 6, title: '诡秘之主', author: '爱潜水的乌贼', description: '蒸汽与机械的浪潮...', cover: 'https://via.placeholder.com/150x200', category: '玄幻', status: '完结' }
    ]
  } finally {
    loading.value = false
